        ...
        for line in file:            
            ...
            if line.strip() == '---------------------------------------------------------------------------------------------------------------------':
                ...
                if len(col[0])<=2:
                    ...
                    texto_tratado.append([cluster, cantidad_de_palabras_clave, porcentaje_de_palabras_clave, principales_palabras_clave])
                else:
                    palabras_clave_limpia=[item.replace('.','') for item in col]
                    principales_palabras_clave=' '.join(palabras_clave_limpia)
                    texto_tratado[len(texto_tratado)-1][3] += ' ' + principales_palabras_clave 

        df=pd.DataFrame(texto_tratado, columns=headers)
